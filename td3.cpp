...
)
{
	...
	//ListeActeurs lActeursTempo(film.acteurs.getCapacite(), film.acteurs.getNElements());
	film->acteurs = ListeActeurs (film->acteurs.getCapacite(), film->acteurs.getNElements());
	//film.acteurs.setElements();
...
//[
ostream& operator << (ostream& afficher, const Film& film)
{
	afficher << "Titre: " << film.titre << endl;
	afficher << "  Réalisateur: " << film.realisateur << "  Année :" << film.anneeSortie << endl;
	afficher << "  Recette: " << film.recette << "M$" << endl;

	afficher << "Acteurs:" << endl;
	for (const auto& acteur : film.acteurs.spanListeActeurs())
		//afficherActeur(*acteur);
		afficher << acteur->nom << acteur->anneeNaissance << acteur->sexe << endl;
	return afficher;
}
